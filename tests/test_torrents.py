...
def test_add_options(test_torrent):
...
def test_download_limit(client, client_func, test_torrent):
    getattr(client, client_func[0])(torrent_hashes=test_torrent.hash, limit=100)
    assert isinstance(getattr(client, client_func[1])(torrent_hashes=test_torrent.hash), TorrentLimitsDictionary)
    check(lambda: getattr(client, client_func[1])(torrent_hashes=test_torrent.hash)[test_torrent.hash], 100)
...
def test_set_share_limits(client, api_version, client_func, test_torrent):
    if is_version_less_than(api_version, '2.0.1', lteq=False):
        with pytest.raises(NotImplementedError):
            getattr(client, client_func)(ratio_limit=2, seeding_time_limit=5, torrent_hashes=test_torrent.hash)
    else:
        getattr(client, client_func)(ratio_limit=2, seeding_time_limit=5, torrent_hashes=test_torrent.hash)
...
def test_set_location(client, api_version, client_func, test_torrent):
    ...
    if is_version_less_than('2.0.1', api_version, lteq=False):
        with pytest.raises(Forbidden403Error):
            getattr(client, client_func)(location='/etc/', torrent_hashes=test_torrent.hash)

    getattr(client, client_func)(location='%s/Downloads/1/' % home, torrent_hashes=test_torrent.hash)
...
def test_set_category(client, client_func, name, test_torrent):
    with pytest.raises(Conflict409Error):
        getattr(client, client_func)(category='/!@#$%^&*(', torrent_hashes=test_torrent.hash)

    client.torrents_create_category(name=name)
    try:
        getattr(client, client_func)(category=name, torrent_hashes=test_torrent.hash)
...
def test_torrents_set_auto_management(client, client_func, test_torrent):
    ...
    getattr(client, client_func)(enable=(not current_setting), torrent_hashes=test_torrent.hash)
...
def test_toggle_sequential_download(client, client_func, test_torrent):
    ...
    getattr(client, client_func)(torrent_hashes=test_torrent.hash)
...
def test_toggle_first_last_piece_priority(client, client_func, test_torrent):
...
def test_set_super_seeding(client, client_func, test_torrent):
    getattr(client, client_func)(enable=False, torrent_hashes=test_torrent.hash)
...
def test_torrents_add_peers(client, api_version, torrent_hash, client_func, peers, test_torrent):
    if is_version_less_than(api_version, '2.3.0', lteq=False):
        with pytest.raises(NotImplementedError):
            getattr(client, client_func)(peers=peers, torrent_hashes=torrent_hash)
    else:
        if all(map(lambda p: ':' not in p, peers)):
            with pytest.raises(InvalidRequest400Error):
                getattr(client, client_func)(peers=peers, torrent_hashes=test_torrent.hash)
        else:
            p = getattr(client, client_func)(peers=peers, torrent_hashes=test_torrent.hash)
...
def test_create_categories(client, api_version, test_torrent, client_func, save_path, name):
    ...
    if is_version_less_than(api_version, '2.1.0', lteq=False):
        with pytest.raises(NotImplementedError):
            getattr(client, client_func)(name=name, save_path=save_path)
        extra_kwargs = {}

    try:
        getattr(client, client_func)(name=name, **extra_kwargs)
...
def test_remove_category(client, api_version, test_torrent, client_func, categories):
    ...
    getattr(client, client_func)(categories=categories)
    if is_version_less_than('2.1.1', api_version):
...
def test_tags(client, api_version):
    if is_version_less_than(api_version, '2.3.0', lteq=False):
        with pytest.raises(NotImplementedError):
            client.torrents_tags()
    else:
        assert isinstance(client.torrents_tags(), TagList)
...
def test_add_tags(client, api_version, test_torrent, client_func, tags):
    if is_version_less_than(api_version, '2.3.0', lteq=False):
        with pytest.raises(NotImplementedError):
            getattr(client, client_func)(tags=tags, torrent_hashes=test_torrent.hash)
    else:
        try:
            getattr(client, client_func)(tags=tags, torrent_hashes=test_torrent.hash)
...
def test_create_tags(client, api_version, client_func, tags):
    if is_version_less_than(api_version, '2.3.0', lteq=False):
        with pytest.raises(NotImplementedError):
            getattr(client, client_func)(tags=tags)
    else:
        try:
            getattr(client, client_func)(tags=tags)

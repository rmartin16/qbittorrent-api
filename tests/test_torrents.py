...
def test_add_delete(client, api_version, client_func):
    ...
    def delete():
        get_func(client, client_func[1])(delete_files=True, torrent_hashes=torrent1_hash)
...
def test_edit_tracker(client, api_version, client_func, orig_torrent):
    if is_version_less_than(api_version, '2.2.0', lteq=False):
        with pytest.raises(NotImplementedError):
            get_func(client, client_func)(torrent_hash=orig_torrent.hash, original_url='127.1.0.1', new_url='127.1.0.2')
    else:
        orig_torrent.add_trackers('127.1.0.1')
        get_func(client, client_func)(torrent_hash=orig_torrent.hash, original_url='127.1.0.1', new_url='127.1.0.2')
        check(lambda: (t.url for t in orig_torrent.trackers), '127.1.0.2', reverse=True)
        getattr(client, 'torrents_remove_trackers')(torrent_hash=orig_torrent.hash, urls='127.1.0.2')
...
def test_rename_file(client, api_version, orig_torrent_hash, orig_torrent, new_name, client_func):
    if is_version_less_than(api_version, '2.4.0', lteq=False):
        with pytest.raises(NotImplementedError):
            getattr(client, client_func)(torrent_hash=orig_torrent_hash, file_id=0, new_file_name=new_name[0])
    else:
        getattr(client, client_func)(torrent_hash=orig_torrent_hash, file_id=0, new_file_name=new_name[0])
...
def test_download_limit(client, client_func, orig_torrent):
    orig_download_limit = get_func(client, client_func[1])(torrent_hashes=orig_torrent.hash)[orig_torrent.hash]
    
    get_func(client, client_func[0])(torrent_hashes=orig_torrent.hash, limit=100)
    assert isinstance(get_func(client, client_func[1])(torrent_hashes=orig_torrent.hash), TorrentLimitsDictionary)
    check(lambda: get_func(client, client_func[1])(torrent_hashes=orig_torrent.hash)[orig_torrent.hash], 100)
    
    # reset download limit
    get_func(client, client_func[0])(torrent_hashes=orig_torrent.hash, limit=orig_download_limit)
    check(lambda: get_func(client, client_func[1])(torrent_hashes=orig_torrent.hash)[orig_torrent.hash], orig_download_limit)
...
def test_upload_limit(client, client_func, orig_torrent):
    orig_upload_limit = get_func(client, client_func[1])(torrent_hashes=orig_torrent.hash)[orig_torrent.hash]
    ...
    # reset upload limit
    get_func(client, client_func[0])(torrent_hashes=orig_torrent.hash, limit=orig_upload_limit)
    check(lambda: get_func(client, client_func[1])(torrent_hashes=orig_torrent.hash)[orig_torrent.hash], orig_upload_limit)
...
def test_set_share_limits(client, api_version, client_func, orig_torrent):
    if is_version_less_than(api_version, '2.0.1', lteq=False):
        with pytest.raises(NotImplementedError):
            get_func(client, client_func)(ratio_limit=2, seeding_time_limit=5, torrent_hashes=orig_torrent.hash)
    else:
        get_func(client, client_func)(ratio_limit=2, seeding_time_limit=5, torrent_hashes=orig_torrent.hash)
        check(lambda: orig_torrent.info.max_ratio, 2)
        check(lambda: orig_torrent.info.max_seeding_time, 5)
        get_func(client, client_func)(ratio_limit=3, seeding_time_limit=6, torrent_hashes=orig_torrent.hash)
    ...
    if is_version_less_than('2.0.1', api_version, lteq=False):
        ...
        if is_version_less_than('2.0.1', api_version, lteq=False):
            with pytest.raises(Forbidden403Error):
                get_func(client, client_func)(location='/etc/', torrent_hashes=new_torrent.hash)

        get_func(client, client_func)(location='%s/Downloads/1/' % home, torrent_hashes=new_torrent.hash)
...
def test_set_category(client, client_func, name, orig_torrent):
    with pytest.raises(Conflict409Error):
        get_func(client, client_func)(category='/!@#$%^&*(', torrent_hashes=orig_torrent.hash)
...
def test_torrents_set_auto_management(client, client_func, orig_torrent):
    ...
    get_func(client, client_func)(enable=(not current_setting), torrent_hashes=orig_torrent.hash)
    check(lambda: orig_torrent.info.auto_tmm, (not current_setting))
    get_func(client, client_func)(enable=False, torrent_hashes=orig_torrent.hash) # leave on False
...
def test_create_categories(client, api_version, orig_torrent, client_func, save_path, name):

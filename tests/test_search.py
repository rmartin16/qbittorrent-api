...
@pytest.mark.parametrize('client_func', ('search_update_plugins', 'search.update_plugins'))
...
def test_enable_plugin(client, api_version, client_func):
    if is_version_less_than(api_version, '2.1.1', lteq=False):
        ...
        try:
            ...
            plugins = get_func(client, client_func[0])
        get_func(client, client_func[1])(plugins=(p['name'] for p in plugins), enable=False)
        check(lambda: (p['enabled'] for p in client.search_plugins()), True, reverse=True, negate=True)
        get_func(client, client_func[1])(plugins=(p['name'] for p in plugins), enable=True)
        check(lambda: (p['enabled'] for p in client.search_plugins()), False, reverse=True, negate=True)


@pytest.mark.parametrize('client_func', (('search_install_plugin', 'search_uninstall_plugin'),
    ...
    if is_version_less_than(api_version, '2.1.1', lteq=False):
        ...
        for _ in range(3):
            try:
                ...
                check(lambda: (p.name for p in client.search.plugins), plugin_name, reverse=True)
                break
        ...
        for _ in range(3):
            try:
                ...
                check(lambda: (p.name for p in client.search.plugins), plugin_name, reverse=True, negate=True)
            except AssertionError:
                pass
...
@pytest.mark.parametrize('client_func', (('search_start', 'search_status', 'search_results', 'search_stop', 'search_delete'),
                                         ('search.start', 'search.status', 'search.results', 'search.stop', 'search.delete')))
def test_search(client, api_version, client_func):
    if is_version_less_than(api_version, '2.1.1', lteq=False):
        with pytest.raises(NotImplementedError):
            get_func(client, client_func[0])()
    else:
        job = get_func(client, client_func[0])(pattern='Ubuntu', plugins='enabled', category='all')
        ...
        check(lambda: get_func(client, client_func[1])(search_id=job['id'])[0]['status'], 'Stopped')
        get_func(client, client_func[4])(search_id=job['id'])
        statuses = get_func(client, client_func[1])()
        assert not statuses


@pytest.mark.parametrize('client_func', (('search_stop', 'search_start'),
                                         ('search.stop', 'search.start')))
def test_stop(client, api_version, client_func):
    if is_version_less_than(api_version, '2.1.1', lteq=False):
        with pytest.raises(NotImplementedError):
            get_func(client, client_func[0])(search_id=100)
    else:
        job = get_func(client, client_func[1])(pattern='Ubuntu', plugins='enabled', category='all')
